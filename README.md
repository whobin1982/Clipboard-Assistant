...
- 图片操作：图片历史支持右键复制图片、导出 PNG；复制图片文字会在 OCR 功能落地前给出明确提示。
...
- 快捷键选择：历史窗口当前可见区域里的前 9 条记录会显示 `⌘1` 到 `⌘9`，滚动后编号会跟着可见记录转移；普通数字、字母和符号都保留给搜索输入。
...
- 图片记录右键可直接复制图片或导出 PNG，导出时会从本地图片归档重新生成普通图片文件。
...
- 使用快捷键 `⌥ + ⌘ + V`：打开剪贴板历史窗口。
- 点击某条历史：按设置执行粘贴或只复制。
- 右键点击图片历史：可复制图片、复制图片文字或导出图片。
- 类型筛选：用历史窗口顶部的“全部 / 文本 / 图片 / 收藏”切换可见记录。
- 上下方向键：在历史窗口中移动选择。
- 回车：确认当前选中记录。
- `⌘1` 到 `⌘9`：快速选择窗口当前可见区域中的第 1-9 条记录。
- Esc：关闭历史窗口。
...
这个版本已经具备日常使用所需的核心能力，并新增图片历史右键操作：复制图片、复制图片文字提示、导出 PNG。后续优先计划围绕批量操作、常用内容复用、图片 OCR 和发布体验继续完善：
